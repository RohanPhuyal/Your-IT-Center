...
<div class="container">
    ...
    <!-- Filter Bar -->
    <!-- <div class="row mb-3">
            <h5 class="mb-0 mr-4">Filter By:</h5>
            <div class="dropdown">
    ...
    <div class="row">
        ...
        $countStmt = $conn->prepare("SELECT COUNT(*) AS total FROM products WHERE category LIKE '%Peripherals' AND stock>0");
        $countStmt->execute();
        ...
        $stmt = $conn->prepare("SELECT * FROM products WHERE category LIKE '%Peripherals' AND stock>0 LIMIT 4 OFFSET ?");
  ...
  <ul class="pagination">
    ...
    <?php for ($i = 1; $i <= $totalPages; $i++){
        if(isset($_GET['page'])&& intval($_GET['page']) === $i){
            echo '<li class="page-item active"><a class="page-link" href="accessories.php?page=' . $i . '">' . $i . '</a></li>';
