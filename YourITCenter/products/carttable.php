        ...
        <div class="col-sm-6 offset-sm-6">
            <table class="table table-striped" style="background-color:#1f2937; color:#fff;">
                <tbody>
                    ...
                    <tr style="background-color:#de5832; color:#fff;">
                        <td>Total:</td>
                        <td>Rs.
                            <?php
            ...
            <div class="text-right">
                ...
                <?php if ($totalPrice > 0): ?>
                    <button class="btn btn-success" data-toggle="modal" data-target="#payModal"
                        style="background-color: #de5832 !important;">Finalize and Pay</button>
                    <!-- Modal -->
                            ...
                            <div class="modal-content" style="background-color:#1f2937;">
                                ...
                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                                    <div class="modal-body">
                                        ...
                                        <button type="submit" name="khalti" class="btn"
                                            style="background-color: #fff !important;"><img src="img/khalti-logo.png"
                                                width="80px" height="35px"> </img></button>
                                        <button type="submit" name="cod" class="btn"
                                            style="background-color: #fff !important;">Cash on Delivery</button>
                                    </div>
                                    <div class=" modal-footer">
            ...
            <?php else: ?>
                <button class="btn btn-success" style="background-color: #de5832 !important;" disabled>Finalize and
                    Pay</button>
            <?php endif; ?>
        </div>
    </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER'] == "http://localhost/YourITCenter/cart.php") {
        if (isset($_SESSION["isLoggedIn"]) && $_SESSION['isLoggedIn'] === false) {
            echo '<script>console.log("http://localhost/YourITCenter/signin.php");</script>';
            echo '<script>window.location.replace("http://localhost/YourITCenter/signin.php");</script>';
        } else {
            if (isset($_POST['khalti'])) {
                ...
                if ($user['phone'] === null || $user['address'] === null || $user['shipping_address'] === null) {
            ...
            if (isset($_POST['cod'])) {
                ...
                if ($user['phone'] === null || $user['address'] === null || $user['shipping_address'] === null) {
                    ...
                    // Generate a random transaction ID starting with 'cod'
                    $transaction_id = 'cod' . uniqid();
                    echo '<script>window.location.href="payments/cod.php?transaction_id=' . $transaction_id . '&product_id=' . $product_id_imploded . '&product_name=' . $product_name_imploded . '&amount=' . $totalAmount . '";</script>';
                }
            }
        }
