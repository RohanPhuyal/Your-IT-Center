...
$countStmt = $conn->prepare("SELECT COUNT(*) AS total FROM orders WHERE customer_id = ?");
...
$stmt_orders = $conn->prepare("SELECT * FROM orders WHERE customer_id = ? LIMIT 4 OFFSET ?");
$stmt_orders->bind_param('ii', $_SESSION['user_id'],$offset);
$stmt_orders->execute();
...
while ($order_row = $result_orders->fetch_assoc()) {
    ...
    echo '<div class="order-container" style="background-color: #1f2937; padding: 15px;">
            <div class="row" style="border-bottom: 1px solid #ccc;">
                ...
                <div class="col-md-4 text-right">
                ';
                if($order_row['order_status']==="Cancelled"||$order_row['order_status']==="Delivered"){
                    echo '';
                }else{
    ...
    foreach ($product_ids as $product_id) {
        ...
        echo '<div class="row mt-3">
                <div class="col-md-2">
                    <img src="' . $firstImage . '" alt="Product Image" class="img-fluid delivery-img">
                </div>
                <div class="col-md-2">
  ...
  <ul class="pagination">
    ...
    <?php for ($i = 1; $i <= $totalPages; $i++){
        if(isset($_GET['page'])&& intval($_GET['page']) === $i){
            echo '<li class="page-item active"><a class="page-link" href="myorders.php?page=' . $i . '">' . $i . '</a></li>';
        }else if(isset($_GET['page'])){
            echo '<li class="page-item"><a class="page-link" href="myorders.php?page=' . $i . '">' . $i . '</a></li>';
        }else if($i===1){
            echo '<li class="page-item active"><a class="page-link" href="myorders.php?page=' . $i . '">' . $i . '</a></li>';
        }else{
            echo '<li class="page-item"><a class="page-link" href="myorders.php?page=' . $i . '">' . $i . '</a></li>';
